...
async function tableExists(db: Kysely<DatabaseSchema>, tableName: string): Promise<boolean> {
	try {
...
async function verifyTableSchema(db: Kysely<DatabaseSchema>, tableName: string, expectedColumns: string[]): Promise<boolean> {
	try {
...
async function createTenantsTable(db: Kysely<DatabaseSchema>): Promise<void> {
	...
	if (await tableExists(db, tableName)) {
		...
		if (isValid) {
			...
			throw new Error(`Table ${tableName} exists but schema is invalid. Manual intervention required.`);
		}
	}
	...
	await db.schema
		...
		.addColumn("id", "text", (col) => col.primaryKey().defaultTo(sql`(hex(randomblob(16)))`))
...
async function createAccountsTable(db: Kysely<DatabaseSchema>): Promise<void> {
	...
	const expectedColumns = ["id", "tenant_id", "name", "status", "parent_account_id", "created_at", "updated_at"];
	...
	await db.schema
		.createTable(tableName)
		.addColumn("id", "text", (col) => col.primaryKey().defaultTo(sql`(hex(randomblob(16)))`))
		.addColumn("tenant_id", "text", (col) => col.notNull().references("tenants.id"))
		.addColumn("name", "text", (col) => col.notNull())
		.addColumn("status", "text", (col) => col.notNull().defaultTo("ACTIVE"))
		.addColumn("parent_account_id", "text", (col) => col.references("accounts.id"))
		.addColumn("created_at", "text", (col) => col.notNull().defaultTo(sql`(datetime('now'))`))
		.addColumn("updated_at", "text", (col) => col.notNull().defaultTo(sql`(datetime('now'))`))
		.execute();
	console.log(`Table ${tableName} created successfully.`);
...
async function createTransactionsTable(db: Kysely<DatabaseSchema>): Promise<void> {
	const tableName = "transactions";
	const expectedColumns = ["id", "account_id", "memo", "currency", "external_id", "status", "created_at", "updated_at"];
...
async function createTransactionEntriesTable(db: Kysely<DatabaseSchema>): Promise<void> {
	const tableName = "transaction_entries";
	const expectedColumns = ["id", "transaction_id", "direction", "amount", "created_at"];
	...
	await db.schema
		.createTable(tableName)
		.addColumn("id", "text", (col) => col.primaryKey().defaultTo(sql`(hex(randomblob(16)))`))
		.addColumn("transaction_id", "text", (col) => col.notNull().references("transactions.id"))
